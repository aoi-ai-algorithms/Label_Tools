...
class YOLOWriter:
    ...
    def BndBox2YoloLine(self, box, classList=[]):
        ...
        return xmin, ymin, dx, dy, boxName
...
class YoloReader:
    ...
    def yoloLine2Shape(self, x1, y1, w, h, label):
    ...
    def parseYoloFormat(self):
        ...
        for bndBox in bndBoxFile:
            x1, y1, w, h, label = bndBox.split(',')
            # Caveat: difficult flag is discarded when saved as yolo format.
            self.addShape(label, int(x1), int(y1), int(x1) + int(w), int(y1) + int(h), False)

...
def kmean_pad_detection(image):
    ...
    def filter_orange_like(image,low_hsv = (0,14,193),high_hsv = (180,255,255)):
        ...
        if (debug): cv2.imshow('image',image)
        return coor
    ...
    def filter_red(image):
        ...
        if (debug): cv2.imshow(window_name,image)
        return coor
    ...
    def kmean(image,k):
        ...
        if (debug): print('centers',centers)
    ...
    def kmean_mask(img_hsv,k,max_H = 0):
        ...
        for x in range(len(centers)):
            ...
            if centers[x][2]> max_H:
                ...
                pass
        if (debug): print('hsv = ',int(lower_hsv[0]),int(lower_hsv[1]),int(lower_hsv[2]))
